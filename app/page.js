...
export default function CassaLido() {
    ...
    try {
      ...
      if (data.success && data.exists) {
        return {
          ...
          id: data.customer?.id, // <-- Questo è l'ID univoco del record sul DB
          exists: true
        };
      }
      return { exists: false };
  ...
  const printWelcomeQR = async (uid, customerName, customerDbId) => {
    try {
      // Uniamo l'UID fisico della carta con l'ID univoco di questa specifica iscrizione nel database.
      // Se la tessera viene resettata, il prossimo cliente avrà un customerDbId diverso!
      ...
      const datiStampa = [
        ...
        `[C][QR size=6]${urlAreaPrivata}\n\n`, // Genera il QR perfetto per la carta da 58mm
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        // 🔥 MAGIA: Mandiamo in stampa il QR passando l'ID univoco generato dal DB
        // Nota: Assicurati che la tua API '/api/register-tag' restituisca l'id del record creato (es: data.id o data.customerId)
  ...
  // 🗑 ... Tutto il resto del codice rimane esattamente identico a prima ...
